...
namespace xUnitTests.Tests.RepositoryTests.ClientRepositoryTests {
	public class AddProductTests : IClassFixture<RepositoryTestFixture> {
		...
		[Fact]
		public void AddProduct_UnkownIDUser_ThrowsException() {
			...
			FluentActions.Invoking(() => client_repository.AddProduct(list_product_dto)).Should().Throw<NotFoundException>().And.Message.Should().Be("List could not be found.");
		}
		[Fact]
		...
		public void AddProduct_ForbiddenStatus_ThrowsException() {
			...
			ClientRepository client_repository = new ClientRepository(cache, context, mapper, messager);
			client_repository.IDUser = 1;
			FluentActions.Invoking(() => client_repository.AddProduct(list_product_dto)).Should().Throw<ForbiddenException>().And.Message.Should().Be("Completed list cannot be changed.");
		}
		[Fact]
		public void AddProduct_ConflictedListProduct_ThrowsException() {
			...
			ClientRepository client_repository = new ClientRepository(cache, context, mapper, messager);
			client_repository.IDUser = 4;
			FluentActions.Invoking(() => client_repository.AddProduct(list_product_dto)).Should().Throw<ConflictException>().And.Message.Should().Be("Product already exists in list.");
		}
		[Fact]
		public void AddProduct_ListProduct_ShouldBeReturn() {
			...
			client_repository.IDUser = 4;
			ListProductDTO list_product_dto = new ListProductDTO { IDList = 5, IDProduct = 5, Quantity = 1 };
			FluentActions.Invoking(() => client_repository.AddProduct(list_product_dto)).Invoke();
