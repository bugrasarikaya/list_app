...
namespace xUnitTests.Tests.ValidatorTests {
	public class CategoryDTOValidatorTests : IClassFixture<RepositoryTestFixture> {
		[Theory]
		[InlineData("")]
		...
		public void Give_Invalid_Input_ShouldReturnError(string name) {
			ValidationResult dto_validation_result = new CategoryDTOValidator().Validate(new CategoryDTO { Name = name });
