...
namespace xUnitTests.Tests.ValidatorTests {
	public class UserDTOValidatorTests : IClassFixture<RepositoryTestFixture> {
		[Theory]
		[InlineData(0, "ninesrodriguez", "password")]
		[InlineData(1, "8cI3dFz", "password")]
		...
		[InlineData(1, "ninesrodriguez", "8cI3dFz")]
		...
		public void Give_Invalid_Input_ShouldReturnError(int id_role, string name, string password) {
			ValidationResult dto_validation_result = new UserDTOValidator().Validate(new UserDTO { IDRole = id_role, Name = name, Password = password });
