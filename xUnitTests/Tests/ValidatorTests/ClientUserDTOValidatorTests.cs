...
namespace xUnitTests.Tests.ValidatorTests {
	public class ClientUserDTOValidatorTests : IClassFixture<RepositoryTestFixture> {
		[Theory]
		[InlineData("", "password")]
		...
		public void Give_Invalid_Input_ShouldReturnError(string name, string password) {
			ValidationResult dto_validation_result = new ClientUserDTOValidator().Validate(new ClientUserDTO { Name = name, Password = password });
