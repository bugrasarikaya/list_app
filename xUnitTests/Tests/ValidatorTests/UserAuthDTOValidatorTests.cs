...
namespace xUnitTests.Tests.ValidatorTests {
	public class UserAuthDTOValidatorTests : IClassFixture<RepositoryTestFixture> {
		[Theory]
		[InlineData("", "password")]
		...
		public void Give_Invalid_Input_ShouldReturnError(string name, string password) {
			ValidationResult dto_validation_result = new UserAuthDTOValidator().Validate(new UserAuthDTO { Name = name, Password = password });
