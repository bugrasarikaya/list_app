...
namespace xUnitTests.Tests.ValidatorTests {
	public class BrandDTOValidatorTests : IClassFixture<RepositoryTestFixture> {
		[Theory]
		[InlineData("")]
		...
		public void Give_Invalid_Input_ShouldReturnError(string name) {
			ValidationResult dto_validation_result = new BrandDTOValidator().Validate(new BrandDTO { Name = name });
