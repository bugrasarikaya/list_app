...
namespace xUnitTests.Tests.ValidatorTests {
	public class StatusDTOValidatorTests : IClassFixture<RepositoryTestFixture> {
		[Theory]
		[InlineData("")]
		...
		public void Give_Invalid_Input_ShouldReturnError(string name) {
			ValidationResult dto_validation_result = new StatusDTOValidator().Validate(new StatusDTO { Name = name });
