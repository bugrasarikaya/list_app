...
namespace xUnitTests.Tests.ValidatorTests {
	public class ClientListPatchDTOValidatorTests : IClassFixture<RepositoryTestFixture> {
		[Theory]
		...
		public void Give_Invalid_Input_ShouldReturnError(int id_category, int id_status, string name, string description) {
			ValidationResult dto_validation_result = new ClientListPatchDTOValidator().Validate(new ClientListPatchDTO { IDCategory = id_category, IDStatus = id_status, Name = name, Description = description });
			dto_validation_result.Errors.Count.Should().BeGreaterThan(0);
		}
		[Theory]
		[InlineData(1, 1, "", "")]
		[InlineData(1, 1, "My List", "My best list.")]
		public void Give_Valid_Input_ShouldNotReturnError(int id_category, int id_status, string name, string description) {
			ValidationResult dto_validation_result = new ClientListPatchDTOValidator().Validate(new ClientListPatchDTO { IDCategory = id_category, IDStatus = id_status, Name = name, Description = description });
