...
namespace list_api.Repository {
	public class RoleRepository : IRoleRepository {
		...
		public RoleViewModel Get(string param_role) { // Getting a role.
			Role role;
			if (int.TryParse(param_role, out int id_role)) role = Supply.ByID<Role>(cache, context, id_role);
			else role = Supply.ByName<Role>(cache, context, param_role);
			return Fill.ViewModel<RoleViewModel, Role>(cache, context, mapper, role);
		}
		public ICollection<RoleViewModel> List() { // Listing all roles.
			...
			foreach (int id in Supply.List<Role>(cache, context).OrderBy(b => b.Name).Select(c => c.ID).ToList()) list_role_view_model.Add(Fill.ViewModel<RoleViewModel, Role>(cache, context, mapper, Supply.ByID<Role>(cache, context, id)));
		...
		public RoleViewModel Update(string param_role, RoleDTO role_dto) { // Updating a role.
			...
			context.SaveChanges();
			return Fill.ViewModel<RoleViewModel, Role>(cache, context, mapper, role_updated);
		}
		public RoleViewModel Patch(string param_role, RolePatchDTO role_patch_dto) { // Patching a role.
			...
			context.SaveChanges();
			return Fill.ViewModel<RoleViewModel, Role>(cache, context, mapper, role_patched);
		}
	}
}