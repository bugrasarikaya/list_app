...
namespace list_api.Repository {
	public class RoleRepository : IRoleRepository {
		...
		public void Delete(int id) { // Deleting a role.
			Role role_deleted = Supply.ByID<Role>(cache, context, id);
			Check.ForeignIDForConflict<User, Role>(cache, context, id);
			context.Roles.Remove(role_deleted);
			context.SaveChanges();
			RedisCache.Recache<Role>(cache, context);
		}
		public RoleViewModel Get(int id) { // Getting a role.
			return mapper.Map<RoleViewModel>(Supply.ByID<Role>(cache, context, id));
		}
		public ICollection<RoleViewModel> List() { // Listing all categories.
			...
			foreach (int id in context.Roles.Select(r => r.ID).ToList()) list_role_view_model.Add(mapper.Map<RoleViewModel>(Supply.ByID<Role>(cache, context, id)));
			return list_role_view_model;
		}
		public RoleViewModel Update(int id, RoleDTO role_dto) { // Updating a role.
			Role role_updated = Supply.ByID<Role>(cache, context, id);
		...
		public RoleViewModel Patch(int id, RolePatchDTO role_patch_dto) { // Patching a role.
			Role role_patched = Supply.ByID<Role>(cache, context, id);
