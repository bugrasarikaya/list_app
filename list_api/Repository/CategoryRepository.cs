	...
	public class CategoryRepository : ICategoryRepository {
		...
		public Category? Delete(int id) { // Deleting a category.
			...
			if (category_deleted != null) {
				if (context.Lists.Any(l => l.IDCategory == id)) throw new InvalidOperationException("Category exists in a list.");
				if (context.Products.Any(p => p.IDCategory == id)) throw new InvalidOperationException("Category exists in a product.");
