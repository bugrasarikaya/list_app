...
namespace list_api.Repository {
	public class ClientRepository : IClientRepository {
		...
		public List CreateList(ClientListDTO list_client_dto) { // Creating a list of client.
			List list_created = new List() { IDCategory = Check.ID<Category>(context, list_client_dto.IDCategory), IDStatus = Supply.ByID<Status>(context, 2).ID, IDUser = Check.ID<User>(context, IDUser), Name = Check.NameForConflict<List>(context, list_client_dto.Name), Description = list_client_dto.Description, DateTime = DateTime.Now };
		...
		public ListViewModel UpdateList(int id_list, ClientListDTO list_client_dto) { // Updating a list of client.
			...
			list_updated.IDCategory = Check.ID<Category>(context, list_client_dto.IDCategory);
			list_updated.IDStatus = Check.ID<Status>(context, list_client_dto.IDStatus);
			list_updated.Name = Check.NameForConflict<List>(context, list_client_dto.Name, IDUser);
		...
		public ClientUserViewModel PatchUser(ClientUserPatchDTO client_user_patch_dto) { // Patching a user.
			User user_patched = Supply.ByID<User>(context, IDUser);
			if (!string.IsNullOrEmpty(client_user_patch_dto.Name)) user_patched.Name = Check.NameForConflict<List>(context, client_user_patch_dto.Name);
